...
| MiniMax (MiniMax-Hailuo-02, legacy video-01/video-01-live2d) | Async submit/poll/download; `MiniMax-Hailuo-02` accepts `first_frame_image` and `last_frame_image` via `first_last_frame` mode | **Live-validated (2026-07-20)**: real submit → poll → download produced a playable MP4 against the real API | **First commercial integration — shipped.** Canonical registry entry, one capability source, project-level selection, capability-gated start/end UI, fixed default-resolution mapping, frame dimension/aspect-ratio validation before spend |
| Veo 3.1 | Initial image, last frame, and up to three asset references; native audio | Strong premium hero-shot provider and clean match for the proposed roles | **Second commercial integration — scaffolded, not live-validated.** Plugs into the same registry/capability/`VideoGenerationAttempt`/shot persistence with no redesign, proving the architecture; needs a real API key/quota test before enabling for users |
...
**Update (2026-07-20): all of the above is complete.** The real model is `MiniMax-Hailuo-02` at `api.minimax.io` — `video-01-keyframe` never existed; it was a placeholder name. `MiniMax-Hailuo-02` supports `first_last_frame` mode directly, auto-selected server-side when an end frame is chosen. Live-validated end-to-end against the real API.
...
**Update (2026-07-20): scaffolded.** `src/providers/video/veo.js` implements the real contract confirmed against the page above: `POST /v1beta/models/{model}:predictLongRunning` with `x-goog-api-key`, an `instances[0]` object carrying `prompt`/`image`/`lastFrame` as `{inlineData:{mimeType,data}}`, a `parameters` object with `aspectRatio` (`16:9`/`9:16` only), `resolution` (`720p`/`1080p`/`4k`), and `durationSeconds` (`"4"|"6"|"8"`, `8` required at 1080p/4k) — then polls `GET /v1beta/{operation_name}` until `done`, reading the result from `response.generateVideoResponse.generatedSamples[0].video.uri`. It resolves through the exact same registry/capability/execution/reconciliation machinery as MiniMax with no special-casing, which is the architectural claim this phase existed to test. It has not been exercised against a real Veo API key/quota — do that, and watch for real-world drift from the documented contract, before enabling it for actual users. `referenceImages` (up to three, character/product identity) is not modeled; that needs the video input-role vocabulary extended.
...
1. **Complete: clean provider/capability sources.** Replaced the single hardcoded video provider default with one canonical registry (`VIDEO_PROVIDERS`, derived from the capability table); collapsed the three overlapping video capability tables into one authoritative source consumed by both server and browser, with a drift-detection test comparing derived output, not just the raw constant.
2. **Complete: fix MiniMax defaults.** The platform's default (`draft`) resolution tier now maps to MiniMax's cheapest real resolution instead of throwing `UNSUPPORTED_MEDIA_OUTPUT` before submission.
3. **Complete: async reconciliation.** Added `src/workers/video-reconciliation-worker.js`, a background interval that drives `reconcileAttempts()` for pending attempts (MiniMax and now Veo) automatically — no manual re-polling.
4. **Complete: project-level provider/model setting.** Platform default → project provider/model preference (`mediaSettings.video.provider`/`.model`) → optional per-generation override. Scene/shot-level override remains deferred.
5. **Complete: capability-gated start/end-frame UI.** `setStartFrame()`/`setEndFrame()` are wired into real per-version controls in the entity modal's history list; the end-frame control only renders when the resolved provider/model's capabilities support it (verified live in-browser for both LTX/stub, which hide it, and MiniMax `MiniMax-Hailuo-02`, which shows it).
6. **Complete: live MiniMax end-to-end validation.** MiniMax is real production code: model corrected to `MiniMax-Hailuo-02` (not a `video-01-keyframe` that never existed) at the real `api.minimax.io` host, `first_last_frame` mode auto-selected when an end frame is chosen, and frame-dimension/aspect-ratio validation before spend. Live-validated against the real API on 2026-07-20: submit → poll (running → completed) → download produced a real playable MP4.
7. **Complete (scaffold): Veo as provider #2 — the architectural test passed.** `src/providers/video/veo.js` implements Veo 3.1's real `predictLongRunning`/operation-polling contract (verified against https://ai.google.dev/gemini-api/docs/veo, 2026-07-20) and plugs into the exact same registry, capability source, `VideoGenerationAttempt` model, execution service, and `reconcileAttempts()` worker as MiniMax — zero changes to `video-generation.service.js`'s control flow, `VideoGenerationAttempt`, or shot persistence were needed beyond registering the provider and its output resolver. A test drives the real adapter (network mocked at the fetch boundary) through submit → reconcile → commit into a shot, proving the neutrality claim. Not yet live-validated against a real Veo API key/quota, unlike MiniMax — do that before enabling it for real users. Deliberately not modeled: Veo's up-to-three `referenceImages` (needs the input-role vocabulary extended, separate scope).
8. **Complete.** `.env.example` documents `MINIMAX_API_KEY`/`MINIMAX_API_HOST`/`MINIMAX_VIDEO_MODEL` and `VEO_API_HOST`/`VEO_VIDEO_MODEL` (Veo reuses the existing `GEMINI_API_KEY`).
9. Roll out behind tenant/admin flags with explicit provider/model selection once Veo is live-validated the same way MiniMax is.
