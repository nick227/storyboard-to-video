...
# if they don't already exist, then creates (or updates, via --force) voice-service-secrets,
# piper-service-secrets, and image-service-secrets in both. Safe to re-run: environments are
# only created if missing, and secrets are recreated with --force so rotated token values take
...
#
# Usage:
#   SPARK_SERVICE_TOKEN=... PIPER_SERVICE_TOKEN=... IMAGE_SERVICE_TOKEN=... ./.github/scripts/modal-bootstrap.sh
...
: "${IMAGE_SERVICE_TOKEN:?IMAGE_SERVICE_TOKEN is required (image-service-secrets)}"
...
for env_name in dev prod; do
  ...
  echo "Creating/updating image-service-secrets in '${env_name}'..."
  modal secret create image-service-secrets \
