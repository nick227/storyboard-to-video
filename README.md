...
- **CI**: GitHub Actions `CI` workflow runs web + voice + piper tests on push/PR. On **push to `main`**, after `voice-service` / `piper-service` jobs pass, it deploys those apps to Modal (`deploy-voice` / `deploy-piper`). Manual fallback: Actions → **Deploy Modal TTS services** → Run workflow. Requires repo secrets `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET`. Note: web CI currently has failing integration tests; that does **not** block Modal deploys (TTS jobs are independent).
