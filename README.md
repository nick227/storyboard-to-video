...
- **`apps/web` (Railway)**: Project `storyboard-to-video`. A **repo-root** `Dockerfile` + `railway.toml` build `apps/web` when Root Directory is unset (GitHub-linked default). Prefer setting dashboard **Root Directory** to `apps/web` later so `apps/web/railway.toml` is the single config.
