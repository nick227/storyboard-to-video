...
- **[apps/image-service/](apps/image-service/)**: Python/FastAPI + SDXL text-to-image (deploys to Modal).
...
| **Visual Prompts** | Camera-agnostic layout prompt (25–60 words, with environment) | Beat + Neighbors + setting context | [shot-planning.service.js](apps/web/src/services/shot-planning.service.js) / [prompt-generation.service.js](apps/web/src/services/prompt-generation.service.js): `VISUAL_PROMPT_RULES` + `CONTINUITY_RULE` |
...
- **`apps/web` (Railway)**: Project `storyboard-to-video` at https://storyboard-to-video.up.railway.app. Dashboard **Root Directory** is `apps/web` (so `apps/web/Dockerfile` + `apps/web/railway.toml` are the live build/deploy config).
  ...
  - **Voice / image services**: Railway's **prod** service must point at the **`prod`** Modal Environment's URLs, never `dev`'s — set `SPARK_TTS_URL` + `SPARK_SERVICE_TOKEN`, `PIPER_SERVICE_URL` + `PIPER_SERVICE_TOKEN`, and `IMAGE_SERVICE_URL` + `IMAGE_SERVICE_TOKEN` to the `prod` Environment's `*.modal.run` URLs (copy them from the Modal dashboard after the first `prod` deploy — see below). Local/dev web instances should use the `dev` Environment's URLs, or a locally-run service (`apps/web/.env.example` defaults Spark to `http://localhost:8001`). Defaults point at localhost (or unset) and will not work in production.
- **Modal Environments (`dev` / `prod`)**: `apps/voice-service`, `apps/piper-service`, and `apps/image-service` each deploy into two separate Modal Environments, not one shared deployment — `dev` is redeployed automatically on every push to `main` that passes tests; `prod` only changes when a human explicitly runs the manual workflow (below). This is what stops routine pushes from ever overwriting whatever is currently serving real customer traffic. One-time setup, before the first deploy of either service:
    ```bash
...
- **`apps/image-service` (Modal)**: SDXL text-to-image on A10G. Image build downloads `stabilityai/stable-diffusion-xl-base-1.0` from Hugging Face (public — no `HF_TOKEN`). Web provider `sdxl` calls `IMAGE_SERVICE_URL` + `IMAGE_SERVICE_TOKEN`. Flux can be added later as a second model on the same service.
- **`apps/alignment-service`**: no deployment target configured yet (no Dockerfile/Railway/Modal config).
- **CI**: GitHub Actions `CI` workflow runs web + voice + piper + image-service tests on push/PR. On **push to `main`**, after those service jobs pass, it deploys to the **`dev`** Modal Environment only (`deploy-voice` / `deploy-piper` / `deploy-image`). **Prod deploys are manual-only**: Actions → **Deploy Modal services (prod)** → Run workflow — this is the sole path that targets the `prod` Environment. Both require repo secrets `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET`.
