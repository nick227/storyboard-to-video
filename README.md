...
- **`apps/web` (Railway)**: Project `storyboard-to-video` at https://web-production-53c15.up.railway.app. **Do this week:** in the Railway dashboard → web service → Settings → set **Root Directory** to `apps/web`, redeploy, then delete the temporary repo-root `Dockerfile` and `railway.toml` (they exist only because Root Directory is currently unset; editing `apps/web/Dockerfile` alone does nothing until the switch).
