...
test('planVisuals actionPrompt rules ask for grounded motion-friendly actions', async () => {
    ...
    textProviders: {
      call: async (_provider, value) => {
        ...
        return JSON.stringify({
          visuals: [
            { sceneNumber: 1, visualPrompt: 'She locks the apartment door and listens.', actionPrompt: 'She locks the apartment door.', videoPrompt: 'She turns the lock and presses her ear to the door as hallway noise swells.' },
            { sceneNumber: 2, visualPrompt: 'Crowd presses against a closed door from outside.', actionPrompt: 'People outside shout and pound the door harder.', videoPrompt: 'Fists hammer the door harder while bodies press in from the hall.' },
            { sceneNumber: 3, visualPrompt: 'She backs away from the door.', actionPrompt: 'She backs away from the door.', videoPrompt: 'She steps back from the door as dust motes drift in the room light.' },
          ],
        });
      },
    },
  ...
  assert.match(request, /Ground it in this scene's narration/);
  assert.match(request, /do not invent props, people, or gestures/);
  assert.match(request, /videoPrompt: one image-to-video motion brief/);
    ...
    textProviders: {
      call: async () => JSON.stringify({
        visuals: [{
          ...
          videoPrompt: 'Derek jolts upright and sweeps the cluttered room with his eyes.',
        }],
      }),
    },
  });
  ...
  const service = createShotPlanningService({
    textProviders: {
      call: async () => {
        calls += 1;
        return JSON.stringify({
          visuals: [{ sceneNumber: 1, visualPrompt: 'New empty scene visual, subject clear.', actionPrompt: 'Subject waits.', videoPrompt: 'Subject waits with a slight continuous shift of weight.' }],
        });
      },
    },
  });
