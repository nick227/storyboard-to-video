...
test('image providers publish explicit reference capabilities', () => {
  assert.deepEqual(Object.keys(IMAGE_PROVIDER_CAPABILITIES).sort(), ['dezgo', 'dezgo_flux', 'gemini', 'local-safetensors', 'openai', 'pixabay', 'sdxl', 'stub']);
  ...
  assert.equal(resolveImageReferencePlan('local-safetensors:biglove-xl1', references(1)).provider, 'local-safetensors');
...
test('browser and server reference planners produce the same manifest-facing plan', async () => {
  ...
  for (const provider of ['gemini', 'openai', 'dezgo', 'dezgo_flux', 'sdxl', 'stub', 'pixabay', 'local-safetensors']) {
