...
const topbarSource = fs.readFileSync(path.join(webRoot, 'public', 'modules', 'topbar.js'), 'utf8');
...
test('the Script, Storyboard, and Timeline tabs appear in that order, with no Narration/Style tabs', () => {
  const markup = renderTopbar('/admin');
...
test('topbar owns shared tab styling and studio retains the download confirmation action', () => {
  const topbarCss = fs.readFileSync(path.join(webRoot, 'public', 'topbar.css'), 'utf8');
  const studio = fs.readFileSync(path.join(webRoot, 'public', 'studio.html'), 'utf8');
