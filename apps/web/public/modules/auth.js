...
// The shared <storyframe-topbar> element is the single place session state is fetched
...
export async function initializeAuth() {
  ...
  const { authenticated, session } = await document.querySelector('storyframe-topbar').sessionReady;
  if (!authenticated) {
    redirectToLogin();
    return null;
  }
