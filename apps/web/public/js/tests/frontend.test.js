import { projectStore, sceneStore, batchStore } from '../core/store.js';
import { createStoryboardRecord } from '../core/persistence.js';
import { loadProtectedAsset, loadedAssets, revokeAllAssets } from '../core/assets.js';
import { loadStyles } from '../studio/ui.js';
import { getZipSummary } from '/js/app.js';
...
async function runTests() {
  try {
    ...
    const renderingSource = await (await fetch('/js/studio/rendering.js')).text();
    const timelineSource = await (await fetch('/js/studio/timeline.js')).text();
    ...
    const workflowsSource = await (await fetch('/js/generation/workflows.js')).text();
    ...
    const apiSource = await (await fetch('/js/core/api.js')).text();
    ...
    const runControllerSource = await (await fetch('/js/studio/run-controller.js')).text();
    const storyboardControllerSource = await (await fetch('/js/studio/storyboard-controller.js')).text();
    const settingsControllerSource = await (await fetch('/js/studio/settings-controller.js')).text();
    ...
    const uiSource = await (await fetch('/js/studio/ui.js')).text();
    const stagesSource = await (await fetch('/js/generation/stages.js')).text();
