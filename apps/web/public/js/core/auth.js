...
// The shared topbar is the single place session state is fetched and rendered
// (including the logout button) -- this just awaits its result rather than
...
export async function initializeAuth() {
  ...
  const { authenticated, session } = await document.querySelector('.sf-topbar').sessionReady;
  if (!authenticated) {
    redirectToLogin();
    return null;
  }
