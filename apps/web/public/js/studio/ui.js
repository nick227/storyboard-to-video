import { projectStore, sceneStore, generationStore, voiceStore, uiStore, batchStore } from '../core/store.js';
import { getCurrentStoryboardRecord, persistStoryboardLibrary, queueSync } from '../core/persistence.js';
import { loadProtectedAsset } from '../core/assets.js';
import { imageShot } from '../core/scene-shots.js';
import { api } from '../core/api.js';
import { previewVoice, stopPreviewVoice, openVoiceLibraryModal } from '../media/voices.js';
import { computeStageStatus, getCachedJobs, getCachedSpend } from '../generation/stages.js';
import {
...
import { renderTokenDetails } from '../billing/token-details.js';
...
function renderGenerationSummary(els, status) {
  ...
  const running = [];
  const failed = [];
  for (const [key, label] of [['planning', 'Planning'], ['images', 'Images'], ['audio', 'Audio'], ['video', 'Video'], ['subtitles', 'Subtitles']]) {
    if (status[key]?.running) running.push(label);
    else if (status[key]?.failed) failed.push(label);
  }
  
  if (downloading) {
    summary = 'Zipping storyboard...';
  } else if (running.length) {
    summary = `Generating: ${running.join(', ')}...`;
  ...
  const isRunning = downloading || running.length > 0;
  els.generationSummaryText.classList.toggle('is-loading', isRunning);
  els.generationSummaryText.setAttribute('aria-busy', String(isRunning));

  if (els.statusPanel) {
    els.statusPanel.classList.toggle('is-running', isRunning);
    els.statusPanel.classList.toggle('is-failed', failed.length > 0 && !isRunning);
    els.statusPanel.classList.toggle('is-ready', !isRunning && failed.length === 0);
  }
}
