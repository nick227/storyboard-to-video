...
async function main() {
  ...
  try {
    ...
    if (config.env.DEZGO_API_KEY) {
      const intent = mergeMediaIntent({ modality: 'image' });
      results.push(await run('dezgo-image', () => images.generate({ provider: 'dezgo_flux', prompt: 'A simple black circle centered on a plain white background.', references: [], title: 'Reconciliation check', output: resolveImageOutput({ provider: 'dezgo_flux', model: 'flux_1_schnell', intent }) })));
    }
