...
async function main() {
  ...
  try {
    ...
    results.push(await run('dezgo', () => {
      const intent = mergeMediaIntent({ modality: 'image' });
      return images.generate({ provider: 'dezgo', prompt: imagePrompt, references: [], title: 'Live suite check', output: resolveImageOutput({ provider: 'dezgo', model: 'text2image', intent }) });
    }));
