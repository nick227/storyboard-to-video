...
function billingRoutes(repository, billing) {
  ...
  router.get('/', asyncRoute(async (req, res) => {
    ...
    const [prices, markups, creditRates, accounts] = await Promise.all([repository.listPrices(), repository.listMarkups(), repository.listCreditRates(), repository.listAccounts()]);
    res.json(jsonSafe({ ok: true, customerChargingEnabled: billing?.chargingEnabled === true, prices, markups, creditRates, accounts }));
  }));
  ...
  router.patch('/prices/:id', asyncRoute(async (req, res) => {
    ...
    res.json(jsonSafe({ ok: true, price: await repository.configurePrice(id, input) }));
  }));
