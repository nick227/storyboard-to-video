...
function createAssetsController({config,projectStore,styles}){return{
 project(req,res){projectStore.read(req.params.projectId,{ownerId:req.auth.tenantId});const file=path.basename(req.params.fileName);if(file!==req.params.fileName)throw new AppError('INVALID_PATH','Invalid asset path',{status:400});const source=path.join(projectStore.assetDir(req.params.projectId,req.params.type),file);if(!fs.existsSync(source))throw new AppError('ASSET_NOT_FOUND','Asset not found',{status:404});res.sendFile(source);},
