...
function createImageGenerationService({ config, styles, provider, projectStore, materializer }) {
  const assetMaterializer = materializer || createAssetMaterializer({
    ...
    cacheDir: config.paths.materialized || path.join(config.paths.generated, '.materialized'),
  });
    ...
    for (const reference of imageShot(scene).referenceBindings || []) {
      ...
      try {
        const asset = projectStore.findAsset
          ...
          : { storageKey: buildProjectAssetStorageKey(projectId, 'images', fileName) };
        const handle = await assetMaterializer.materialize(asset.storageKey);
        releases.push(handle.release);
        paths.push({ localPath: handle.path, path: reference.path, source: 'scene', role: normalizeReferenceRole(reference.role) });
  ...
  return {
    async preflight(input, context = {}) {
      ...
      try {
        ...
        await resolved.releaseMaterialized();
      }
    },
