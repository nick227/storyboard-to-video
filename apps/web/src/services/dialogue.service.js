...
const ATTRIBUTION_RULE = 'Do not habitually close a line or scene with a "[Name] said" attribution tag — this is a crutch, not a requirement. One narrator voice reads every line, so the listener doesn\'t need a tag to tell speakers apart. Let most dialogue simply stand on its own, as if performed, with no tag at all. When a name is already established, prefer a pronoun over repeating it, and prefer letting the line hang unattributed over adding "he/she said." Use an explicit attribution tag only on the rare line where the speaker would otherwise be genuinely ambiguous, and vary its wording and placement rather than defaulting to a trailing "said."';
...
function sourceOfTruthRule(enrich) {
  const elaboration = enrich
    ? ' Narration may add sensory or atmospheric detail that brings an implied setting, transition, or dynamic to life — that is encouraged —'
    : ' Narration should not add sensory or atmospheric detail beyond what the source states —';
  return `The source text below is the only authoritative source for this scene's content. The scene action is secondary interpretation guidance only.${elaboration} but it must never introduce new plot events, characters, or dialogue that aren't in the source text.`;
}
...
function createDialogueService({ textProviders, generationCache }) {
  async function generate({ scenes, provider, fallbackPolicy = 'local', enrich = true }) {
    ...
    for (const batch of batches) {
      ...
      try {
        ...
        // results[index] for every scene in this batch already carries fallback+usedFallback:true
        // from the initial seeding above — nothing further to do here.
      }
    }
  ...
  async function regenerate({ scene, sceneIndex, instruction = '', provider, fallbackPolicy = 'local', enrich = true, tenantId, bypassCache = false }) {
    ...
    // don't wire a cache) or explicitly requests a new variation via bypassCache.
    const fingerprintInput = tenantId ? {
      ...
      source: JSON.stringify({ scriptFragment: scene.scriptFragment, beat: scene.beat || '', instruction }),
      settings: { enrich },
    } : null;
