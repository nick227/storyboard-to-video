...
function createVoiceService(config,getCancellation,audioProvider={}){
 ...
 return{
 async elevenLabsVoices(){if(!config.env.ELEVENLABS_API_KEY)throw new Error('ELEVENLABS_API_KEY missing');const response=await fetch('https://api.elevenlabs.io/v1/voices',{headers:{'xi-api-key':config.env.ELEVENLABS_API_KEY},signal:signal(config.env.AUDIO_PROVIDER_TIMEOUT_MS||60000,getCancellation)});if(!response.ok)await throwResponse('elevenlabs',response);return(await response.json()).voices?.map(v=>({voiceId:v.voice_id,label:v.name,previewUrl:v.preview_url||null}))||[];},
 piperVoices(){return config.piperVoices.map(id=>({voiceId:id,label:PIPER_VOICE_CATALOG.find(v=>v.id===id)?.label||id}));},
 async piperPreview(voiceId){if(!config.piperVoices.includes(voiceId))throw new AppError('UNKNOWN_VOICE','Unknown Piper voice',{status:404});return audioProvider.piperPreview(voiceId);},
