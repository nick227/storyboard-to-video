...
class GenerationQueue {
  ...
  async restore() { for (const saved of await (this.store?.loadAndInterrupt?.() || [])) this.jobs.set(saved.id, saved); }

  async add(type, projectId, task, { sceneId, tenantId, userId, idempotencyKey } = {}) {
    await this.ready;
    const controller = new AbortController();
    const job = { id: crypto.randomUUID(), type, projectId: projectId || null, sceneId: sceneId || null, tenantId: tenantId || null, userId: userId || null, idempotencyKey: idempotencyKey || null, status: 'queued', createdAt: new Date().toISOString(), controller };
  ...
  async cancelProject(projectId) {
    await this.ready;
    return Promise.all([...this.jobs.values()].filter((job) => job.projectId === projectId && !this.isTerminal(job.status)).map((job) => this.cancel(job.id)));
  }
  ...
  async run({ job, task }) {
    ...
    try {
      ...
      job.status = 'succeeded'; job.result = result; await this.store?.save(job); job.resolve(result);
    } catch (error) {
      if (job.status !== 'cancelled') { job.status = 'failed'; job.error = { code: error.code || 'GENERATION_FAILED', message: error.message }; await this.store?.save(job); job.reject(error); }
    } finally {
      job.finishedAt ||= new Date().toISOString();
      try { await this.store?.save(job); }
      finally {
        this.running -= 1;
        await this.prune();
        this.drain();
  ...
  async prune() {
    ...
    await Promise.all(finished.slice(0, Math.max(0, finished.length - this.retain)).map((job) => { this.jobs.delete(job.id); return this.store?.delete(job.id); }));
  }
