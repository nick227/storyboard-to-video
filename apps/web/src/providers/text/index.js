...
function geminiParts(prompt, references) {
  ...
  for (const [index, reference] of (references || []).entries()) {
    ...
    parts.push(
      { text: `REFERENCE ${index + 1} — ${instruction}` },
      { inline_data: { mime_type: mime(file), data: fs.readFileSync(file).toString('base64') } },
    );
  }
  return parts;
}

/** Prefer non-thought text parts so thinking models don't pollute JSON extraction. */
function geminiCandidateText(data) {
  const parts = data?.candidates?.[0]?.content?.parts || [];
  const visible = parts.filter((part) => part.text && !part.thought).map((part) => part.text);
...
function createTextProviders(config, getCancellation, usageTracker, providerAdmission) {
  async function openai(prompt) {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${config.env.OPENAI_API_KEY}` },
      body: JSON.stringify({ model, input: prompt }),
      signal: signal(config.env.TEXT_PROVIDER_TIMEOUT_MS || 60_000, getCancellation),
    });
    ...
    return providerResult({
      ...
      providerRequestId: providerRequestId(response, data),
      usage: rawUsage ? {
      ...
      rawUsage,
      measurementStatus: rawUsage ? 'observed' : 'unavailable',
    });
  }

  async function gemini(prompt, references = []) {
    if (!config.env.GEMINI_API_KEY) throw new Error('GEMINI_API_KEY missing');
    const model = config.env.GEMINI_TEXT_MODEL || 'gemini-3.5-flash';
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${config.env.GEMINI_API_KEY}`;
    const response = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{ parts: geminiParts(prompt, references) }],
        generationConfig: { temperature: 0.7, responseMimeType: 'application/json' },
      }),
      signal: signal(config.env.TEXT_PROVIDER_TIMEOUT_MS || 60_000, getCancellation),
    });
    if (!response.ok) await throwResponse('gemini', response);
    const data = await response.json();
    const rawUsage = data.usageMetadata || null;
    return providerResult({
      output: geminiCandidateText(data),
      provider: 'gemini',
      ...
      usage: rawUsage ? {
        ...
        outputTokens: (rawUsage.candidatesTokenCount || 0) + (rawUsage.thoughtsTokenCount || 0),
        candidateTokens: rawUsage.candidatesTokenCount || 0,
        thinkingTokens: rawUsage.thoughtsTokenCount || 0,
        totalTokens: rawUsage.totalTokenCount || 0,
        serviceTier: rawUsage.serviceTier || 'standard',
      } : {},
      rawUsage,
      measurementStatus: rawUsage ? 'observed' : 'unavailable',
    });
  ...
  function call(provider, prompt, references = []) {
    const model = provider === 'stub'
      ? 'stub-text-v1'
      : provider === 'openai'
        ? (config.env.OPENAI_TEXT_MODEL || 'gpt-4.1-mini')
        : (config.env.GEMINI_TEXT_MODEL || 'gemini-3.5-flash');
    const operation = () => (provider === 'stub'
      ? Promise.resolve(providerResult({
        output: '',
        provider: 'stub',
        model,
        usage: { inputCharacters: String(prompt).length },
        measurementStatus: 'not_applicable',
      }))
      : provider === 'openai' ? openai(prompt) : gemini(prompt, references));
    const tracked = () => (usageTracker
      ? usageTracker.execute({
        modality: 'text',
        provider,
        model,
        inputMetadata: { promptCharacters: String(prompt).length, referenceCount: references.length },
      }, operation)
      : operation());
  ...
  return { call, geminiParts, geminiCandidateText };
}
