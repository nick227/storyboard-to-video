...
function createImageProviders(config, textProviders, getCancellation, usageTracker) {
  ...
  function generate({ provider, prompt, references = [], title }) {
    ...
    const operation = () => provider === 'stub'
      ? Promise.resolve(providerResult({ output: { buffer: stubImage(prompt, title), mimeType: 'image/svg+xml', extension: 'svg' }, provider: 'stub', model: models.stub, usage: { images: 1 }, measurementStatus: 'not_applicable' }))
      : provider === 'openai' ? openai(prompt, references) : provider === 'dezgo' ? dezgo(prompt, references) : gemini(prompt, references.slice(0, 14));
