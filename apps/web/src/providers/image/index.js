...
function createImageProviders(config, textProviders, getCancellation, usageTracker, providerAdmission) {
  ...
  async function dezgo(prompt, references = [], output) {
    ...
    let body = new URLSearchParams({ ...common, width: String(width), height: String(height) });
    if (references.length) {
    ...
    const response = await fetch(url, { method: 'POST', headers, body, signal: signal(config.env.IMAGE_PROVIDER_TIMEOUT_MS || 180_000, getCancellation) });
    if (!response.ok) await throwResponse('dezgo', response); return providerResult({ output: { buffer: Buffer.from(await response.arrayBuffer()), mimeType: 'image/png', extension: 'png' }, provider: 'dezgo', model: config.env.DEZGO_MODEL || 'text2image', providerRequestId: providerRequestId(response), settings: { output, steps: Number(config.env.DEZGO_STEPS || 25), guidance: Number(config.env.DEZGO_GUIDANCE || 7), sampler: config.env.DEZGO_SAMPLER || 'euler_a', mode: references.length ? 'image_to_image' : 'text_to_image', ...(references.length ? { strength: Number(config.env.DEZGO_REFERENCE_STRENGTH || 0.65) } : {}) }, usage: { images: 1, ...estimatedUsage(output), steps: Number(config.env.DEZGO_STEPS || 25), guidance: Number(config.env.DEZGO_GUIDANCE || 7), sampler: config.env.DEZGO_SAMPLER || 'euler_a' }, rawUsage: { inputSeed: response.headers.get('x-input-seed') || null }, measurementStatus: 'estimated' });
  }
